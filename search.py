...
def search(
    ...
    for idx in index:
        ...
        for query in queries:
            ...
            hnsw_index, _ = build_index(data, M=M, ef_construction=ef_c, threads_num=thr_num)
...
def main():
    ...
    parser.add_argument("--output_dir", default="./search_results", help="Папка для сохранения лог файлов")
    ...
    if (args.ef or args.thr_num) and not (args.ef and args.thr_num):
        parser.error("Если указан хотя бы один из параметров --ef или --thr_num, то необходимо указать оба.")

    ef = args.ef
    thr_num = args.thr_num

    idx = [Path(args.idx_file)] if args.idx_file else sorted([
        file.resolve() for file in Path(args.idx_dir).iterdir() if file.is_file() and file.suffix.lower() == ".idx"
    ])
    dsc = [Path(args.dsc_file)] if args.dsc_file else sorted([
        file.resolve() for file in Path(args.dsc_dir).iterdir() if file.is_file() and file.suffix.lower() == ".dsc"
    ])
